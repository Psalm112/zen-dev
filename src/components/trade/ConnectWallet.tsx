...
import { useWallet, useWalletBalance, useWalletNetwork } from '../../context/WalletContext';
            ...
            {currencies.map((currency) => (
              <button
                ...
                className={`w-full px-4 py-3 text-left hover:bg-[#3A3D45] transition-colors duration-200 first:rounded-t-lg last:rounded-b-lg ${
                  selectedCurrency === currency.code ? 'bg-[#3A3D45] text-[#ff343f]' : 'text-white'
                }`}
              >
...
const BalanceDisplay = memo<BalanceDisplayProps>(({ variant }) => {
  ...
  const displayBalance = useMemo(() => {
    switch (selectedCurrency) {
      ...
      case 'CELO': return formatted.celo;
      case 'FIAT': return formatted.fiat;
      default: return formatted.usdt;
    }
...
const VerificationFlow = memo<VerificationFlowProps>(({ type, onBack, onSuccess }) => {
  ...
  const placeholder = type === 'email' ? 'Enter your email address' : 'Enter your phone number';
  const title = type === 'email' ? 'Connect with Email' : 'Connect with Phone';
    ...
    >
      ...
      {step === 'input' ? (
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            <inputIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
            <input
...
const WalletDetails = memo<WalletDetailsProps>(({ isOpen, onToggle }) => {
  ...
  const openExplorer = useCallback(() => {
    ...
    window.open(`${explorerUrl}/address/${account}`, '_blank', 'noopener,noreferrer');
  }, [account, isTestnet]);
          ...
          >
            <div className="p-4 pt-0 space-y-4">
              ...
              <div className="p-3 bg-[#212428] rounded-lg">
                <p className="text-xs text-gray-400 uppercase tracking-wide">Wallet Type</p>
...
const ConnectWallet = memo<ConnectWalletProps>(({ 
  ...
  const [currentView, setCurrentView] = useState<'connect' | 'email' | 'phone'>('connect');
  ...
  // Connection handlers
  const handleWalletConnect = useCallback(async (type: string) => {
    try {
      clearError();
  ...
  }, [availableWallets, connectWallet, deviceInfo.preferredConnection, clearError]);
  ...
  const handleSocialConnect = useCallback(async (provider: string) => {
    try {
      clearError();
      switch (provider) {
        case 'google':
  ...
  }, [connectWithGoogle, connectWithApple, connectWithPasskey, connectAsGuest, deviceInfo, clearError]);
      ...
      <AnimatePresence mode="wait">
        {currentView === 'connect' ? (
          ...
          >
            ...
            {/* Alternative Wallets */}
            <div className="space-y-3">
              <p className="text-gray-400 text-sm text-center">Or connect with</p>
            ...
            <div className="space-y-3">
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-2 bg-[#2A2D35] text-gray-400">Quick Connect</span>
                </div>
              </div>
