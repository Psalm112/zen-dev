...
const DatePickerField: React.FC<DatePickerFieldProps> = ({
  ...
  // Item height constant - used for consistent measurements
  ...
  // Reset temp date when opening the picker and center the selected values
  useEffect(() => {
    if (showDatePicker) {
      setTempDate({ ...selectedDate });
      // Center the selected values after a small delay to ensure DOM is ready
      setTimeout(() => {
  ...
  // Effect: whenever tempDate changes, scroll to center
  useEffect(() => {
    if (showDatePicker) {
      scrollToCenter(true);
    }
  ...
  // Improved scroll to element function with more precise calculations
  const scrollToElement = (
    ...
    if (element) {
      ...
      // Calculate scroll position to center the element exactly
      const containerHeight = container.clientHeight;
      const elementHeight = ITEM_HEIGHT;

      // More precise calculation: element's position relative to container
      // minus half of container height plus half of element height
      const scrollTop =
  ...
  // Improved scroll snap function with more precise calculations
  const handleScroll = (
    containerRef: React.RefObject<HTMLDivElement | null>
  ) => {
    if (!containerRef.current) return;
    ...
    }, 100); // Shorter timeout for more responsive feel

    container.dataset.timeoutId = timeoutId.toString();
  };

  // Calculate spacer height to ensure proper centering
  const getSpacerHeight = () => {
    // This creates equal space above and below to center the selected item
        ...
        {showDatePicker && (
          ...
          >
            ...
            >
              ...
              {/* Date picker rows - horizontal layout */}
